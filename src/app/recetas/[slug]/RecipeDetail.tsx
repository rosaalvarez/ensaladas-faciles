      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="grid md:grid-cols-2 gap-10">
          <div>
            ...
            <div className="bg-gray-50 rounded-2xl p-6">
              {recipe.ingredients.map((ing, i) => (
                <label key={i} className={`flex items-center gap-3 py-2 cursor-pointer ${locked && i > 2 ? 'blur-sm pointer-events-none' : ''}`}>
                  <input type="checkbox" checked={checkedIngredients.has(i)} onChange={() => toggleIngredient(i)} className="w-5 h-5 rounded text-green-500 focus:ring-green-500" />
            ...
            {!locked && (
              <button onClick={handleAddToList} className={`mt-4 w-full py-3 rounded-full font-semibold transition ${added ? 'bg-green-600 text-white' : 'bg-green-500 text-white hover:bg-green-600'}`}>
          ...
          <div>
            ...
            <div className="space-y-6">
              {recipe.steps.map((step, i) => (
                <div key={i} className={`flex gap-4 ${locked && i > 0 ? 'blur-sm' : ''}`}>
